...
int SEMmesh::getNumberOfPointsPerElement() const {
  return (((ny == 1) ? (orderx + 1) * (orderz + 1)
                     : (orderx + 1) * (ordery + 1) * (orderz + 1)));
}
...
// get ny
int SEMmesh::getNy() const { return ny; }

// get nz
int SEMmesh::getNz() const { return nz; }

// get dx
int SEMmesh::getDx() const { return hx; }

// get dy
int SEMmesh::getDy() const { return hy; }

// get dz
int SEMmesh::getDz() const { return hz; }

int SEMmesh::getEx() const { return ex; }
int SEMmesh::getEy() const { return ey; }
int SEMmesh::getEz() const { return ez; }

// get coord in one direction
std::vector<float> SEMmesh::getCoordInOneDirection(const int &order,
                                                   const int &nCoord,
                                                   const int &h,
...
// Initialize nodal coordinates.
void SEMmesh::nodesCoordinates_opt(arrayReal &nodeCoordsX, arrayReal &nodeCoordsZ,
                               arrayReal &nodeCoordsY) const {
...
int SEMmesh::getElementNumberFromPoints(const float &x, const float &y,
                                        const float &z) const {
  int eX = 0, eY = 0, eZ = 0;
  for (int i = 0; i < ex; i++) {
    if (x >= i * hx && x < (i + 1) * hx)
...
void SEMmesh::getListOfInteriorElements(
...
void SEMmesh::saveSnapShot(const int indexTimeStep, const int i1,
  ...
  std::vector<std::vector<float>> grid =
      projectToGrid(numberOfNodes, inputVector);
  fstream snapFile;
