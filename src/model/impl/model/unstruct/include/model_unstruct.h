 ...
 public:
    ...
    PROXY_HOST_DEVICE ModelUnstruct(const ModelUnstructData<FloatType, ScalarType>& data)
        : order_(data.order_),
          ...
          lx_(data.lx_), ly_(data.ly_), lz_(data.lz_),
     ...
     */
    PROXY_HOST_DEVICE
    ScalarType getNumberOfElements() const final {  // Added const, removed virtual
        return n_element_;
    }
     ...
     */
    PROXY_HOST_DEVICE
    ScalarType getNumberOfNodes() const final {  // Added const, removed virtual
        return n_node_;
    }
