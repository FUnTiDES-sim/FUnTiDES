...
class BoundaryLayers
{
 public:
   ...
   */
  void Initialize(const fdtd_options& opt)
  {
    // TODO: Extract from opt.boundary or similar
    // For now, set to reasonable defaults
    ntaperx_ = ntapery_ = ntaperz_ = 10;
    ndampx_ = ndampy_ = ndampz_ = 20;

    ComputeBoundaryRegions(opt.grid.nx, opt.grid.ny, opt.grid.nz);
  }
   ...
   */
  void ComputeBoundaryRegions(int nx, int ny, int nz)
  {
    // Example: divide domain into 6 regions per axis
    // x1-x2: left boundary, x3-x4: interior, x5-x6: right boundary
    x1_ = 0;
    x2_ = ndampx_;
    x3_ = ndampx_ + ntaperx_;
    x4_ = nx - ntaperx_ - ndampx_;
    x5_ = nx - ndampx_;
    x6_ = nx;

    y1_ = 0;
    y2_ = ndampy_;
    y3_ = ndampy_ + ntapery_;
    y4_ = ny - ntapery_ - ndampy_;
    y5_ = ny - ndampy_;
    y6_ = ny;

    z1_ = 0;
    z2_ = ndampz_;
    z3_ = ndampz_ + ntaperz_;
    z4_ = nz - ntaperz_ - ndampz_;
    z5_ = nz - ndampz_;
    z6_ = nz;
  }
