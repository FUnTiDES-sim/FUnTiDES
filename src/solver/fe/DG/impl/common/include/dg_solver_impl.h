...
void DGsolver<ORDER, INTEGRAL_TYPE, MESH_TYPE, IS_MODEL_ON_NODES, PHYSICS>::computeBoundaryDampingAndInterfaceFlux_Team(
  ...
  // A team owns one face. The flat policy launched a single thread per face, which starved the
  // device of parallelism long before it ran out of registers. Clamped to what the backend can
  // provide: a host backend caps the team size at its thread count, and asking for more aborts at
  // launch. A TeamThreadRange is correct at any team size.
  int const team_size = std::min<int>(kFaceTeamSize, ExecSpace::concurrency());
