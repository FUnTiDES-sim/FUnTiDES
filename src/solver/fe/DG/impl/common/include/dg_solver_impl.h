  ...
  Kokkos::parallel_for(
      "Solver DG Update Field Acoustic", kNumElem, KOKKOS_LAMBDA(const int e) {
        ...
        INTEGRAL_TYPE::computeStiffnessTerm(
            elementCoords, [&](const int, const int, const int) {},
        ...
        for (int faceId = 0; faceId < 6; ++faceId) {
          ...
          if (!face_connectivity_local.isBoundaryFace(f)) {
            ...
            int const neighbor_e =
                e_is_owner ? face_connectivity_local.elemNeighbor(f) : face_connectivity_local.elemOwner(f);
            model::CubicFace const neighbor_local_face = static_cast<model::CubicFace>(
                e_is_owner ? face_connectivity_local.localFaceNeighbor(f) : face_connectivity_local.localFaceOwner(f));
            real_t const gamma = computeSIPGPenalty<ORDER>(faceCoords, elementCoords, penalty_local);

            INTEGRAL_TYPE::computeInterfaceFluxTerm(
                faceCoords, elementCoords, faceId, [&](const int i, const int j, const int k, const real_t val) {
            ...
            for (int i = 0; i < knumNodesPerFace; ++i) {
              int const ei = model::faceLocalToElemLocal(static_cast<model::CubicFace>(faceId), i, ORDER);
              int const ei_perm = model::faceLocalToElemLocal(neighbor_local_face,
