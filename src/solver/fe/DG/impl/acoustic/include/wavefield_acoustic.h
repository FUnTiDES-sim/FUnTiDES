...
struct WavefieldAcoustic : public Wavefield {
  ...
  PROXY_HOST_DEVICE
  WavefieldAcoustic(ARRAY_REAL_VIEW pnGlobalPrev, ARRAY_REAL_VIEW pnGlobalCurr)
  ...
  PROXY_HOST_DEVICE
  ARRAY_REAL_VIEW getCurrentField(int i) const override { return m_pnGlobalCurr; }

  PROXY_HOST_DEVICE
  ARRAY_REAL_VIEW getPreviousField(int i) const override { return m_pnGlobalPrev; }

  void swap() override { std::swap(m_pnGlobalPrev, m_pnGlobalCurr); }

  void swapWithRotation(ARRAY_REAL_VIEW& prevPrevBuffer, int i) override {
