...
void bind_wavefield_acoustic(py::module_ &m)
{
  ...
  py::class_<WavefieldAcoustic, Wavefield, std::shared_ptr<WavefieldAcoustic>>(
      ...
      .def(py::init([](py::array_t<value_type> pn_global_prev_py,
          ...
          // Safely wrap the NumPy pointers into Unmanaged Host Views
          Kokkos::View<value_type*, Kokkos::LayoutRight, Kokkos::HostSpace, Kokkos::MemoryTraits<Kokkos::Unmanaged>>
...
void bind_wavefield_elastic(py::module_ &m)
{
  ...
  py::class_<WavefieldElastic, Wavefield, std::shared_ptr<WavefieldElastic>>(
      m, "WavefieldElastic")
      .def(py::init([](py::array_t<value_type> uxn_prev_py, py::array_t<value_type> uxn_curr_py,
                       py::array_t<value_type> uyn_prev_py, py::array_t<value_type> uyn_curr_py,
                       py::array_t<value_type> uzn_prev_py, py::array_t<value_type> uzn_curr_py) {

          // Helper lambda to map a 1D NumPy array to a Kokkos Device View
          auto wrap_and_copy = [](py::array_t<value_type> arr, const std::string& name) {
          ...
          return new WavefieldElastic(
              wrap_and_copy(uxn_prev_py, "uxn_prev"), wrap_and_copy(uxn_curr_py, "uxn_curr"),
              wrap_and_copy(uyn_prev_py, "uyn_prev"), wrap_and_copy(uyn_curr_py, "uyn_curr"),
              wrap_and_copy(uzn_prev_py, "uzn_prev"), wrap_and_copy(uzn_curr_py, "uzn_curr")
          );
