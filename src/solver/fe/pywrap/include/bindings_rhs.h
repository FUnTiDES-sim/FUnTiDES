...
py::class_<RhsAcoustoElastic, Rhs, std::shared_ptr<RhsAcoustoElastic>>(
      ...
      .def(
          py::init(
              [](Kokkos::Experimental::python_view_type_t<ARRAY_REAL_VIEW> acoustic_term_py, // CORRIGÉ
                 Kokkos::Experimental::python_view_type_t<VECTOR_INT_VIEW> element_py,       // CORRIGÉ
                 Kokkos::Experimental::python_view_type_t<ARRAY_REAL_VIEW> weights_py,      // CORRIGÉ
                 Kokkos::Experimental::python_view_type_t<ARRAY_REAL_VIEW> elastic_termx_py, // CORRIGÉ
                 Kokkos::Experimental::python_view_type_t<ARRAY_REAL_VIEW> elastic_termy_py, // CORRIGÉ
                 Kokkos::Experimental::python_view_type_t<ARRAY_REAL_VIEW> elastic_termz_py) { // CORRIGÉ

                // Les lambdas internes doivent aussi utiliser les types pybind11 corrects
                // Souvent on utilise simplement py::array_t<real_type> pour plus de clarté
                
                auto wrap_and_copy_2d_real =
                    [](py::array_t<real_type> arr, const std::string& name) {
                      auto buf = arr.request();
                      if (buf.ndim != 2) throw std::runtime_error(name + " must be 2D");
                ...
                auto wrap_and_copy_1d_int =
                    [](py::array_t<int_type> arr, const std::string& name) {
                      auto buf = arr.request();
                      if (buf.ndim != 1) throw std::runtime_error(name + " must be 1D");
