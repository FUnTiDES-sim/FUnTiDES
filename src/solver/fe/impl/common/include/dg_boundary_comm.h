 ...
 public:
  ...
  void exchange(arrayReal& field) const {
    ...
    for (int m = 0; m < n_ghost_; ++m)
      for (int d = 0; d < n_dof_; ++d)
        send_right[m * n_dof_ + d] = h_field(right_elems_[m], d);

    for (int m = 0; m < n_ghost_; ++m)
      for (int d = 0; d < n_dof_; ++d)
        send_left[m * n_dof_ + d] = h_field(left_elems_[m], d);

    // Phase 1: send right slice → right_rank, recv from left_rank → left ghost
    MPI_Sendrecv(send_right.data(), buf_size, MPI_FLOAT, right_rank, 0,
                 recv_left.data(),  buf_size, MPI_FLOAT, left_rank,  0,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);

    // Phase 2: send left slice → left_rank, recv from right_rank → right ghost
    MPI_Sendrecv(send_left.data(),  buf_size, MPI_FLOAT, left_rank,  1,
                 recv_right.data(), buf_size, MPI_FLOAT, right_rank, 1,
                 MPI_COMM_WORLD, MPI_STATUS_IGNORE);

    if (rank_ > 0)
      for (int m = 0; m < n_ghost_; ++m)
        for (int d = 0; d < n_dof_; ++d)
          h_field(n_local_ + m, d) = recv_left[m * n_dof_ + d];

    if (rank_ < size_ - 1)
      for (int m = 0; m < n_ghost_; ++m)
        for (int d = 0; d < n_dof_; ++d)
          h_field(n_local_ + n_ghost_ + m, d) = recv_right[m * n_dof_ + d];
