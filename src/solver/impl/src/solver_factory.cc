...
static std::unique_ptr<SEMSolverBase> make_sem_solver(int order, meshType mesh, modelLocationType modelLocation,
                                                      physicType physic)
{
  ...
  switch (mesh)
  {
    case Struct:
      ...
      {
        case Acoustic:
          return orderDispatch(order, [isModelOnNodes](auto orderIC) -> std::unique_ptr<SEMSolverBase> {
            ...
            if (isModelOnNodes)
            {
              return std::make_unique<SEMsolverAcoustic<ORDER, SelectedIntegral, MeshT, true>>();
            }
            else
            {
              return std::make_unique<SEMsolverAcoustic<ORDER, SelectedIntegral, MeshT, false>>();
            }
          });
        case Elastic:
          return orderDispatch(order, [isModelOnNodes](auto orderIC) -> std::unique_ptr<SEMSolverBase> {
            ...
            if (isModelOnNodes)
            {
              return std::make_unique<SEMsolverElastic<ORDER, SelectedIntegral, MeshT, true>>();
            }
            else
            {
              return std::make_unique<SEMsolverElastic<ORDER, SelectedIntegral, MeshT, false>>();
            }
          });
      }
    case Unstruct:
                                            ...
                                            physicType const physicType, int const order)
{
  if (methodType == SEM)
  {
    ...
    {
      case MAKUTU:
        return make_sem_solver<IntegralType::MAKUTU>(order, mesh, modelLocation, physicType);
        // case SHIVA:
