                             ...
                             const ARRAY_REAL_VIEW &pnGlobal )
{
  ...
  int I = 0;
  if constexpr (std::is_same_v<INTEGRAL_TYPE, typename IntegralTypeSelector<ORDER, 0>::type>) {
      constexpr int coords_size = (ORDER+1) * (ORDER+1) * (ORDER+1);
      float coords[coords_size][3];
      for (int k = 0; k <= ORDER; ++k) {
          for (int j = 0; j <= ORDER; ++j) {
              for (int i = 0; i <= ORDER; ++i) {
                  int nodeIdx = m_mesh.globalNodeIndex(elementNumber, i, j, k);
                  coords[I][0] = m_mesh.nodeCoord(nodeIdx, 0);
                  coords[I][1] = m_mesh.nodeCoord(nodeIdx, 1);
                  coords[I][2] = m_mesh.nodeCoord(nodeIdx, 2);
                  ++I;
              }
          }
      }
      ...
      INTEGRAL_TYPE::computeMassMatrixAndStiffnessVector(
          elementNumber,m_mesh.getNumberOfPointsPerElement(), coords,
          m_precomputedIntegralData,
          massMatrixLocal, pnLocal, Y);
  } else {
      // REDUCED / CORNER-BASED
      float coords[8][3];

      constexpr int nodes_corner[2] = {0, ORDER};
      ...
      const int nPointPerElement = m_mesh.getNumberOfPointsPerElement();
      INTEGRAL_TYPE::computeMassMatrixAndStiffnessVector(
          elementNumber, nPointPerElement, coords,
          m_precomputedIntegralData,
          massMatrixLocal, pnLocal, Y);
  }

  auto const inv_model2 =
    1.0f / (m_mesh.getModelVpOnElement(elementNumber) * m_mesh.getModelVpOnElement(elementNumber));
  for (int i = 0; i < m_mesh.getNumberOfPointsPerElement(); ++i) {
    int x = i % dim;
    int z = (i / dim) % dim;
    int y = i / (dim * dim);
