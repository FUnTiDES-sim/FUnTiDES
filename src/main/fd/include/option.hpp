...
public:
    ...
    static void bind_cli(cxxopts::Options& opts, FDProxyOptions& o) {
        // Grid options
        opts.add_options("Grid")
            ("nx", "Number of grid points on X", cxxopts::value<int>(o.grid.nx))
            ...
            ("mesh", "Mesh type (cartesian)", cxxopts::value<std::string>(o.grid.mesh));

        // Stencil options
        opts.add_options("Stencil")
            ("lx", "Half stencil size X", cxxopts::value<int>(o.stencil.lx))
            ("ly", "Half stencil size Y", cxxopts::value<int>(o.stencil.ly))
            ("lz", "Half stencil size Z", cxxopts::value<int>(o.stencil.lz))
            ("implem", "Stencil implementation (remez|taylor)", cxxopts::value<std::string>(o.stencil.implem));

        // Source options
        opts.add_options("Source")
            ("xs", "Source X position", cxxopts::value<int>(o.source.xs))
            ...
            ("sourceOrder", "Source time derivative order", cxxopts::value<int>(o.source.sourceOrder));

        // Velocity options
        opts.add_options("Velocity")
            ("vmin", "Minimum velocity", cxxopts::value<float>(o.velocity.vmin))
            ("vmax", "Maximum velocity", cxxopts::value<float>(o.velocity.vmax))
            ("fileModel", "Velocity model file", cxxopts::value<std::string>(o.velocity.fileModel));

        // Time stepping options
        opts.add_options("Time")
            ("timeStep", "Time step (0=auto)", cxxopts::value<float>(o.time.timeStep))
            ("timeMax", "Maximum simulation time", cxxopts::value<float>(o.time.timeMax))
            ("method", "Time stepping method", cxxopts::value<std::string>(o.time.method));

        // Boundary options
        opts.add_options("Boundary")
            ("usePML", "Use PML boundaries", cxxopts::value<bool>(o.boundary.usePML))
            ("pmlSize", "PML layer thickness", cxxopts::value<int>(o.boundary.pmlSize))
            ("spongeSize", "Sponge layer thickness", cxxopts::value<int>(o.boundary.spongeSize))
            ("spongeAlpha", "Sponge strength", cxxopts::value<float>(o.boundary.spongeAlpha));

        // Output options
        opts.add_options("Output")
            ("saveSnapShots", "Enable snapshot saving", cxxopts::value<bool>(o.output.saveSnapShots))
            ("snapShotInterval", "Steps between snapshots", cxxopts::value<int>(o.output.snapShotInterval));
