...
public:
  ...
  */
  ...
  {
      const float alpha = (SEMQkGLBasisFunctions::parentSupportCoord<SEMinfo>( q ) + 1.0 ) / 2.0;
  ...
  */
  PROXY_HOST_DEVICE 
  constexpr static float  jacobianCoefficient1D(  const int q, const int i, const int k, const int dir )
  {
      if( i == dir )
      {
  ...
  */
   ...
   PROXY_HOST_DEVICE
   void jacobianTransformation( float const (&X)[8][3],
                                float ( & J )[3][3] ) const
   {
      constexpr int SEQK=8;
      constexpr int SEQJ=3;
  ...
  */
  ...
  template<int qa,int qb,int qc>
  PROXY_HOST_DEVICE 
  void computeBMatrix( float const (&X)[8][3],
                       float (& J)[3][3],
                       float (& B)[6] ) const
  {
      //jacobianTransformation<qa, qb, qc>( X, J );
