...
class Qk_Hexahedron_Lagrange_GaussLobatto {
 public:
  ...
  /**
   * @brief C-PML elastic stiffness via sum factorization.
   *
   * Two-sided C-PML generalization of computeElasticStiffnessSumFact for the
   * second-order elastic equation. The trial field is the displacement tensor
   * gradient H[k][t] = ∂u_t/∂x_k, so the gradient stretch needs 9 memory
   * variables (3 directions × 3 components) and the divergence stretch another
   * 9 (the elastic flux is a vector over force components, so the divergence
   * memory variable is per direction × component):
   *
   *   H_stretched[k][t] = (H[k][t] − ψ[k][t]) / κ_k
   *   div_stretched[j][f] = (divF[j][f] − χ[j][f]) / κ_j
   *
   * with the memory variables advanced by the first-order convolution (Wang,
   * Lee & Teixeira 2006, eq. 21):
   *
   *   ψ[k][t] ← coef0_k ψ[k][t] + coef1_k H[k][t]
   *   χ[j][f] ← coef0_j χ[j][f] + coef1_j divF[j][f]
   *
   * The memory variables are used at their current time level (ψ^n, χ^n) in
   * the stretched quantities and advanced to n+1 afterwards — the standard
   * C-PML timing. With a zero profile (ψ=χ=0, κ=1) the kernel reduces exactly
   * to computeElasticStiffnessSumFact.
   *
   * @tparam FUNC1     Constitutive callback with signature
   *   `void(int qa, int qb, int qc,
   *         real_t const (&J_inv)[3][3],
   *         real_t const (&H_stretched)[3][3],
   *         real_t (&flux)[3][3])`
   *   where @p H_stretched is the stretched physical gradient
   *   (H[k][t] − ψ[k][t])/κ_k and @p flux[p][f] the unscaled flux (scaled by
   *   w·|detJ| inside the kernel). All physics stays in the caller.
   * @tparam FUNC_PML  Callable
   *   `void(int qa, int qb, int qc, real_t (&kappa)[3], real_t (&coef0)[3],
   *   real_t (&coef1)[3])` returning the per-direction stretching κ and the
   *   convolution coefficients at the GLL point.
   ...
   * @param f_local    Force accumulation buffer, shape [3][numNodes],
   *                   accumulated in-place.
   * @param mem_local  Memory variables (18 × numNodes): rows 0-8 = gradient
   *                   ψ[k*3+t], rows 9-17 = divergence χ[j*3+f]; read as
   *                   ψ^n/χ^n and written back as ψ^{n+1}/χ^{n+1}.
...
PROXY_HOST_DEVICE void Qk_Hexahedron_Lagrange_GaussLobatto<GL_BASIS>::computeElasticStiffnessSumFactPML(
  ...
  // For each quad point: compute reference gradients of all 3 displacement
  // components, invert the Jacobian, stretch the physical gradient with the
  // psi memory variables (used at their current level, then advanced), call
  // the constitutive callback with the stretched gradient, and scale/store.
  triple_loop<num1dNodes, num1dNodes, num1dNodes>([&](auto const icqa, auto const icqb, auto const icqc) {
    constexpr int qa = decltype(icqa)::value;
    constexpr int qb = decltype(icqb)::value;
    constexpr int qc = decltype(icqc)::value;
    ...
    for (int k = 0; k < 3; ++k)
      for (int t = 0; t < 3; ++t)
        H[k][t] = J.data[0][k] * grad_u_ref[0][t] + J.data[1][k] * grad_u_ref[1][t] + J.data[2][k] * grad_u_ref[2][t];
    ...
    // Stretched gradient using psi at its current level, then advance psi.
    // C-PML (Komatitsch & Martin 2007): with s_k = κ_k + d_k/(α_k + iω),
    //   1/s_k = (1/κ_k)[1 − (d_k/κ_k)/(α_k + d_k/κ_k + iω)]
    // so the stretched gradient is (H − ψ)/κ with the memory variable
    //   dψ/dt + (α + d/κ) ψ = (d/κ) H
    // advanced by the exact convolution (Wang/Lee/Teixeira 2006):
    //   ψ ← coef0·ψ + coef1·H,  coef1 = (d/κ)/a · (1 − coef0).
    // The MINUS sign inside the parentheses is essential: with a plus sign the
    // steady-state stretched gradient vanishes (ψ → H), turning the PML into a
    // hard wall.
    ...
    // gradient.
    real_t flux[3][3] = {{0}};
    func1(qa, qb, qc, J.data, H_stretched, flux);
  ...
  // The three reference divergences are computed separately (D^T·F^ξ,
  // D^T·F^η, D^T·F^ζ), each a vector over force components, then each
  // direction's component is stretched by its own chi memory variable.
  triple_loop<num1dNodes, num1dNodes, num1dNodes>([&](auto const icia, auto const icib, auto const icic) {
    constexpr int ia = decltype(icia)::value;
    constexpr int ib = decltype(icib)::value;
    constexpr int ic = decltype(icic)::value;
