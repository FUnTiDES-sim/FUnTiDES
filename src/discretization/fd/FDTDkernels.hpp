...
{
  // innerpoints
  int inner3D(FDTDGRIDS &myGrids, FDTDMODELS &myModels, int &ca, int &cb, const int x3, const int x4, const int y3,
              const int y4, const int z3, const int z4, arrayReal const &PN_Global) const
  {
    CREATEVIEWINNER
  ...
  int pml3D(FDTDGRIDS &myGrids, FDTDMODELS &myModels, int &ca, int &cb, const int x3, const int x4, const int y3,
            const int y4, const int z3, const int z4, arrayReal const &PN_Global) const
  {
    CREATEVIEWPML
    LOOP3DHEAD(x3, y3, z3, x4, y4, z4)
    ...
    phi[IDX3(i, j, k)] =
        (phi[IDX3(i, j, k)] -
         ((eta[IDX3_eta1(i + 1, j, k)] - eta[IDX3_eta1(i - 1, j, k)]) *
              (pnGlobal(IDX3_l(i + 1, j, k), cb) - pnGlobal(IDX3_l(i - 1, j, k), cb)) * myGrids.hdx_2 +
          (eta[IDX3_eta1(i, j + 1, k)] - eta[IDX3_eta1(i, j - 1, k)]) *
              (pnGlobal(IDX3_l(i, j + 1, k), cb) - pnGlobal(IDX3_l(i, j - 1, k), cb)) * myGrids.hdy_2 +
          (eta[IDX3_eta1(i, j, k + 1)] - eta[IDX3_eta1(i, j, k - 1)]) *
              (pnGlobal(IDX3_l(i, j, k + 1), cb) - pnGlobal(IDX3_l(i, j, k - 1), cb)) * myGrids.hdz_2)) /
  ...
  {
    // inner points
    inner3D(myGrids, myModels, ca, cb, myGrids.x3, myGrids.x4, myGrids.y3, myGrids.y4, myGrids.z3, myGrids.z4,
            myModels.pnGlobal);
  ...
  int computeOneStepPML(FDTDGRIDS &myGrids, FDTDMODELS &myModels, int &ca, int &cb) const
  {
    ...
    // left
    pml3D(myGrids, myModels, ca, cb, myGrids.x1, myGrids.x2, myGrids.y3, myGrids.y4, myGrids.z3, myGrids.z4,
          myModels.pnGlobal);
    // inner points
    inner3D(myGrids, myModels, ca, cb, myGrids.x3, myGrids.x4, myGrids.y3, myGrids.y4, myGrids.z3, myGrids.z4,
            myModels.pnGlobal);
    // right
    pml3D(myGrids, myModels, ca, cb, myGrids.x5, myGrids.x6, myGrids.y3, myGrids.y4, myGrids.z3, myGrids.z4,
          myModels.pnGlobal);
    // back
  ...
  // innerpoints
  int applySponge(FDTDGRIDS &myGrids, FDTDMODELS &myModels, int &ca, int &cb, const int x3, const int x4, const int y3,
                  const int y4, const int z3, const int z4, arrayReal const &PN_Global) const
  {
    CREATEVIEWSPONGE
    LOOP3DHEAD(x3, y3, z3, x4, y4, z4)
