...
struct FdtdKernels
{
  ...
  // add RHS term
  int addRHS(const int itSample, int &cb, 
             int const &nx, int const &ny, int const &nz, 
             int const &lx, int const &ly, int const &lz,
             int const &xs, int const &ys, int const &zs, 
             vectorReal const &vp,vectorReal const &RHSTerm,
  ...
  int pml3D(int & ca, int & cb,
            ...
            arrayReal const &pnGlobal ) const
   {
        ...
        phi[IDX3(i,j,k)]=(phi[IDX3(i,j,k)]
          -((eta[IDX3_eta1(i+1,j,k)]-eta[IDX3_eta1(i-1,j,k)])*(pnGlobal(IDX3_l(i+1,j,k),cb)-pnGlobal(IDX3_l(i-1,j,k),cb))*hdx_2
           +(eta[IDX3_eta1(i,j+1,k)]-eta[IDX3_eta1(i,j-1,k)])*(pnGlobal(IDX3_l(i,j+1,k),cb)-pnGlobal(IDX3_l(i,j-1,k),cb))*hdy_2
           +(eta[IDX3_eta1(i,j,k+1)]-eta[IDX3_eta1(i,j,k-1)])*(pnGlobal(IDX3_l(i,j,k+1),cb)-pnGlobal(IDX3_l(i,j,k-1),cb))*hdz_2))
