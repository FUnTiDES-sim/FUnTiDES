...
#define IDX3_l(i,j,k) ((NZ+2*LZ)*(NY+2*LY)*((i)+LX) + (NZ+2*LZ)*((j)+LY) +
((k)+LZ)) #define IDX3_eta1(i,j,k) ((NZ+2)*(NY+2)*((i)+1) + (NZ+2)*((j)+1) +
((k)+1))
*/
...
constexpr size_t sqrt_helper(size_t n, size_t lo, size_t hi)
{
  return (lo == hi)
             ...
             : ((n / ((lo + hi + 1) / 2) < ((lo + hi + 1) / 2))
                    ? sqrt_helper(n, lo,
                                  ((lo + hi + 1) / 2) - 1)  // search lower half
                    : sqrt_helper(n, ((lo + hi + 1) / 2),
                                  hi));  // search upper half
}
// constexpr integer sqrt
constexpr size_t sqrt(size_t n) { return sqrt_helper(n, 0, n / 2 + 1); }
...
constexpr size_t lesser_of_squarest_factor_pair_helper(size_t n, size_t guess)
{
  return ((n / guess) * guess == n)
             ...
             : lesser_of_squarest_factor_pair_helper(
                   n, guess - 1);  // continue searching
}
...
{
  return (n == 0) ? 0  // return 0 in the 0 case
...
#define RAJANestedLoop(x3, y3, z3, x4, y4, z4)                               \
  ...
  using EXEC_POL = RAJA::KernelPolicy<RAJA::statement::CudaKernelFixedAsync< \
      x_block_sz * y_block_sz * z_block_sz,                                  \
      RAJA::statement::For<                                                  \
          0, RAJA::cuda_global_size_z_direct<x_block_sz>,                    \
          RAJA::statement::For<                                              \
              1, RAJA::cuda_global_size_y_direct<y_block_sz>,                \
              RAJA::statement::For<                                          \
                  2, RAJA::cuda_global_size_x_direct<z_block_sz>,            \
...
#define RAJANestedLoop(x3, y3, z3, x4, y4, z4)                              \
  ...
  RAJA::TypedRangeSegment<int> JRange(y3, y4);                              \
  RAJA::TypedRangeSegment<int> IRange(x3, x4);                              \
  using EXEC_POL = RAJA::KernelPolicy<RAJA::statement::HipKernelFixedAsync< \
      x_block_sz * y_block_sz * z_block_sz,                                 \
      RAJA::statement::For<                                                 \
          0, RAJA::hip_global_size_z_direct<x_block_sz>,                    \
          RAJA::statement::For<                                             \
              1, RAJA::hip_global_size_y_direct<y_block_sz>,                \
              RAJA::statement::For<                                         \
                  2, RAJA::hip_global_size_x_direct<z_block_sz>,            \
                  RAJA::statement::Lambda<0, RAJA::Segs<0, 1, 2>>>>>>>;     \
     RAJA::kernel<EXEC_POL>( RAJA::make_tuple(IRange, JRange, KRange), [=] __device__ (int i, int j, int k)
#endif
#endif
...
#define LOOP3DHEAD(x3, y3, z3, x4, y4, z4)                              \
  _Pragma("omp parallel for collapse(3)") for (int i = x3; i < x4; ++i) \
  {                                                                     \
    for (int j = y3; j < y4; ++j)                                       \
    {                                                                   \
      for (int k = z3; k < z4; ++k)                                     \
...
#define FDFENCE                                                                \
  ...
  RAJA::forall<RAJA::omp_parallel_for_exec>(RAJA::RangeSegment(0, myGrids.nx), \
                                            [pnGlobalView](int i) {});
#elif defined(USE_KOKKOS)
