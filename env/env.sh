#export _TPL_ROOT_DIR=~/src/proxyApp/buildInstall/installTPLwithsm89Kokkos4.2.01
export _TPL_ROOT_DIR=~/CCPP/proxyApp/buildInstall/installTPL
export RAJA_DIR=${_TPL_ROOT_DIR}/raja/lib/cmake/raja
export CAMP_DIR=${_TPL_ROOT_DIR}/raja/lib/cmake/camp
export CHAI_DIR=${_TPL_ROOT_DIR}/chai/share/chai/cmake/
export UMPIRE_DIR=${_TPL_ROOT_DIR}/chai/lib/cmake/umpire/
#kokkos_install_dir=~/src/proxyApp/buildInstall/installTPLwithsm89Kokkos4.2.01/kokkos
kokkos_install_dir=~/CCPP/proxyApp/buildInstall/installTPL/kokkos
export KOKKOS_DIR=$kokkos_install_dir/lib/cmake/Kokkos
export KOKKOS_INCLUDE_DIR=$kokkos_install_dir/include
