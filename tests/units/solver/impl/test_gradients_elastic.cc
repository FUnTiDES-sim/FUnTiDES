...
class GradientsElasticTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    grad_rho_elem = allocateVector<vectorReal>(num_elements, "grad_rho_elem");
    grad_lambda_elem = allocateVector<vectorReal>(num_elements, "grad_lambda_elem");
    grad_mu_elem = allocateVector<vectorReal>(num_elements, "grad_mu_elem");

    // Allocate node-based gradient vectors
    grad_rho_node = allocateVector<vectorReal>(num_nodes, "grad_rho_node");
    grad_lambda_node = allocateVector<vectorReal>(num_nodes, "grad_lambda_node");
    grad_mu_node = allocateVector<vectorReal>(num_nodes, "grad_mu_node");
  ...
  size_t num_elements;
  size_t num_nodes;
  vectorReal grad_rho_elem;
  vectorReal grad_lambda_elem;
  vectorReal grad_mu_elem;
  vectorReal grad_rho_node;
  vectorReal grad_lambda_node;
  vectorReal grad_mu_node;
};
