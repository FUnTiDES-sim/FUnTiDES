...
BENCHMARK_TEMPLATE_METHOD_F(SolverStructFixture, FEInit)(benchmark::State &state)
{
  // Prepare
...
BENCHMARK_TEMPLATE_METHOD_F(SolverStructFixture, OneStep)(benchmark::State &state)
{
  // Prepare
  auto model = this->createModel();
...
BENCHMARK_FOR_ALL_ORDERS(SolverStructFixture, FEInit, BuilderConfig,
  ->ArgsProduct({{0, 1}, {SolverFactory::implemType::MAKUTU}})->Unit(benchmark::kMillisecond))
BENCHMARK_FOR_ALL_ORDERS(SolverStructFixture, OneStep, BuilderConfig,
  ->ArgsProduct({{0, 1}, {SolverFactory::implemType::MAKUTU}})->Unit(benchmark::kMillisecond))
